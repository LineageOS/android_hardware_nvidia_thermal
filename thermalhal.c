...
ssize_t thermal_init()
{
    ...
    // check ro.hardware and then ro.product.name
    if (__system_property_get(HARDWARE_TYPE_PROP, hw_name)
            || __system_property_get(NAME_TYPE_PROP, prod_name)) {
        if (parse_thermal_config_xml(hw_name) && parse_thermal_config_xml(prod_name)) {
